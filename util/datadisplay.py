...
def plotCostFunction(regParam):
    ...
    inputVectors, targets = Load.labelled_xnor()
    ...
    for x, y in zip(theta1, theta2):
        for value in x:
            theta = array(
                [-30.0, 20.0, 20.0, value, y[0], -20.0, -10, 20.0, 20.0])
            cost = nn._NeuralNetwork__costFunction(theta, inputVectors,
                                                   targets, regParam)[0]
            J = append(J, cost)
...
def plotCostFunctionVsRegParam():
    ...
    for regParam in regParamSet:
        trainResults.append(
            nn._NeuralNetwork__costFunction(vectorTheta, inputVectors, targets,
                                            regParam)[0])
